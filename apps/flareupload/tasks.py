...
def verify_excel_file(self, file_path, file_info, file_name, user_id):
    ...
    for s_idx, (sheet_name, df) in enumerate(all_sheets_list):
        ...
        send_message(
            "success", f"Starting {sheet_name} sheet", progress=int(sheet_base_prg))
        if df.empty:
            send_message("error", f"Sheet '{sheet_name}' is empty", int(
                sheet_base_prg + 2))
        ...
        if missing_columns:
            send_message("error", f"Sheet '{sheet_name}': Missing required columns: {', '.join(missing_columns)}", int(
                sheet_base_prg + 4))
            notifications_count += 1
            hasErrors = True
...
def process_uploaded_file(self, cache_key, file_id=None, filename=None, task_id=None):
    ...
    if not file_bytes:
        raise RuntimeError(f"Cache miss for key {cache_key}")
    print(file_bytes)
    # 🔹 Read Excel
